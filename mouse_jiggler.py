...
Windows: SendInput API | macOS: CoreGraphics (Quartz) Events
"""
...
# ============================================================================
# macOS CoreGraphics (Quartz) Implementation
# ============================================================================
    ...
    try:
        ...
        from Quartz import (
            ...
            kCGMouseEventDeltaX,
            kCGMouseEventDeltaY,
            kCGEventTargetProcessID
        )
    except ImportError:
        print("❌ pyobjc-framework-Quartz wird benötigt. Installiere mit:")
        print("   uv pip install pyobjc-framework-Quartz")
        sys.exit(1)
    
    def get_cursor_position() -> Tuple[int, int]:
        """Ermittelt aktuelle Mausposition via CoreGraphics."""
        event = CGEventGetLocation(None)
        return (int(event.x), int(event.y))
    
    def send_mouse_move_relative(dx: int, dy: int):
        """
        Sendet relative Mausbewegung via CGEvent.
        
        Args:
            dx: Relative Bewegung in X-Richtung (Pixel)
            dy: Relative Bewegung in Y-Richtung (Pixel)
        """
        # Erstelle ein Mausbewegungs-Event mit Delta-Werten
        event = CGEventCreateMouseEvent(
            None,  # Keine Quell-Event
            kCGEventMouseMoved,
            (0, 0),  # Position wird ignoriert bei relativer Bewegung
            0  # Button (0 = keine Taste)
        )
        
        # Setze die Delta-Werte für relative Bewegung
    ...
    def send_mouse_move_absolute(x: int, y: int):
        """
        Sendet absolute Mausbewegung via CGEvent.
        
        Args:
            x: Absolute X-Position
            y: Absolute Y-Position
        """
        # CoreGraphics arbeitet mit absoluten Koordinaten
        event = CGEventCreateMouseEvent(
            None,
            kCGEventMouseMoved,
            (x, y),
    ...
    def get_status_message(self) -> str:
        ...
        if in_window:
            ...
            if next_activation:
                ...
                return f"⏸️  Inaktiv ({start_str}-{end_str}) - Aktuell: {current_str} - Nächste Aktivierung: {next_activation.strftime('%H:%M')} (in {wait_minutes:.0f} Min.)"
...
class MouseJiggler:
    ...
    def __init__(self, time_window: Optional[TimeWindow] = None):
        ...
        print(f"   Windows: SendInput API | macOS: CoreGraphics Events")
...
def test_send_input():
    """Testet die SendInput/CGEvent Funktionalität."""
...
Beispiele:
  ...
  %(prog)s -d 120 --time-start 08:00 --time-end 18:00 -r 4
    ...
    parser.add_argument("-d", "--duration", type=float, default=60,
                       help="Dauer in Minuten (0 = unendlich, Standard: 60)")
